  ...
  spawn(config: AgentProcessConfig): void {
    ...
    this.process = spawn(command, args, {
      ...
      env: {
        ...process.env,
        ...env,
        // NOTE: Do NOT add OPENCODE_ENABLE_QUESTION_TOOL here. Enabling the question
        // tool in ACP mode causes the agent to block indefinitely after the user
        // answers a question — the answer arrives via a new session/prompt turn and
        // the pending question's Deferred is never resolved.
      },
    });
